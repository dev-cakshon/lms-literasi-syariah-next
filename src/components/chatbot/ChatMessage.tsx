...
export const ChatMessage = ({ role, content, isStreaming = false }: ChatMessageProps) => {
  ...
  return (
    <div className={`flex gap-3 animate-in fade-in slide-in-from-bottom-2 duration-500 ${isBot ? '' : 'flex-row-reverse'}`}>
      {/* Avatar */}
      <div
      ...
      <div
        className={`flex-1 min-w-0 max-w-[85%] rounded-2xl p-4 shadow-sm break-words ${
          isBot ? 'bg-white border border-gray-100 text-gray-800' : 'bg-primary-600 text-white'
        }`}
      >
          ...
          >
            <ReactMarkdown
              ...
              components={{
                ...
                strong: ({ children }) => (
                  <strong className='font-bold text-inherit opacity-90'>{children}</strong>
                ),
                ul: ({ children }) => (
                ...
                td: ({ children }) => (
                  <td className='px-4 py-2 text-xs text-gray-700 border-b'>{children}</td>
                ),
                code: ({ children }) => (
                  <code
                    className={`px-1.5 py-0.5 rounded-md font-mono text-[11px] font-medium ${
                      isBot ? 'bg-gray-100 text-primary-700' : 'bg-primary-700 text-white'
                    }`}
                  >
                    {children}
                  </code>
