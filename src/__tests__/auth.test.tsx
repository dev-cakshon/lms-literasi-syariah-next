...
describe('F1 — Autentikasi', () => {
  ...
  it('TC-F.01: Register dengan data valid (nama, email baru, password ≥ 6 karakter)', async () => {
    ...
    await userEvent.type(screen.getByLabelText(/nama lengkap/i), 'Test Student');
    ...
    await waitFor(() => {
      expect(mockSignUp).toHaveBeenCalledWith('Test Student', 'new@test.com', 'password123');
    });
  ...
  it('TC-F.02: Register dengan email yang sudah terdaftar', async () => {
    const mockSignUp = jest
      .fn()
      .mockRejectedValue(
        new Error('Email sudah terdaftar. Silakan gunakan email lain atau masuk (login) ke akun Anda.'),
      );
    mockUseAuth.mockReturnValue({
      user: null,
      userProfile: null,
  ...
  it('TC-F.05: Login dengan password salah', async () => {
    ...
    const { FirebaseError } = require('firebase/app') as {
      FirebaseError: new (code: string, msg: string) => Error & { code: string };
    };
    const wrongPasswordError = new FirebaseError('auth/wrong-password', 'Wrong password');
