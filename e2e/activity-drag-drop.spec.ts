...
test.describe('F8 — Drag & Drop (E2E)', () => {
  test.beforeEach(() => {
    if (!TEST_COURSE_ID || !TEST_DRAG_DROP_ACTIVITY_ID) {
      test.skip(
        true,
        'E2E_TEST_COURSE_ID / E2E_DRAG_DROP_ACTIVITY_ID not configured',
      );
    }
  });
  ...
  test('TC-F.30: Student menyelesaikan semua pasangan lalu menekan Submit — layar hasil muncul', async ({
    studentPage: page,
  }) => {
    ...
    let remainingItems = await page
      .locator('[data-testid^="pool-item-"]')
      .all();
    ...
    while (remainingItems.length > 0) {
      ...
      const categoryZones = await page
        .locator('[data-testid^="category-zone-"]')
        .all();
